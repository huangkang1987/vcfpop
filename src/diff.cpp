...
#include "vcfpop.h"
	...
	TARGET double FST::FstEstimator(POP** grps, int n, int e, double* each, double* buf)
	{
		switch (e)
		{
	...
	TARGET double FST::Fst_Nei1973(POP** grps, int n, double* each, double* buf)
	{
	...
	TARGET double FST::Fst_Weir1984(POP** grps, int n, double* each)
	{
	...
	TARGET double FST::Fst_Hudson1992(POP** grps, int n, double* each, double* buf)
	{
	...
	TARGET double FST::Fst_Slatkin1995(POP** grps, int n, double* each, double* buf)
	{
	...
	/* Hedrick 2005 G'st */
	TARGET double FST::Fst_Hedrick2005(POP** grps, int n, double* each, double* buf)
	{
		//weight by pop size chakraborty 1974, allow ad
		double DA = 0, DB = 0;
		double* pi = buf;
	...
	/* Jost 2008 D */
	TARGET double FST::Fst_Jost2008(POP** grps, int n, double* each, double* buf)
	{
		//weight by pop size chakraborty 1974, allow ad
		double DA = 0, DB = 0;
		double* pi = buf;
	...
	TARGET double FST::Fst_Huang2021_homo(POP** grps, int n, int layer, bool isiam, double* each)
	{
	...
	/* Huang 2021 Fst estimator based on multi-level amova */
	TARGET double FST::Fst_Huang2021_aneu(POP** grps, int n, int layer, bool isiam, bool sumss, double* each, double* buf)
	{
	...
	TARGET void FST::ColumnPrint(FILE* fout)
	{
		byte* estimator = fst_estimator_val;
	...
	TARGET void FST::MatrixPrint(FILE* fout, FST* Fst, int n, int type)
{
	byte* estimator = fst_estimator_val;
