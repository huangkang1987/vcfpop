...
#include "vcfpop.h"
	...
	TARGET void RELATEDNESS::ColumnPrintHeader()
	{
	...
	TARGET void RELATEDNESS::MatrixPrintMatrixHeader(int k, int n)
	{
	...
	TARGET void RELATEDNESS::MatrixPrintRowHeader(int k, int i)
	{
		if (relatedness_estimator_val[k] == 0) return;
	...
	TARGET double RELATEDNESS::RelatednessEstimator(int k, IND* x, IND* y)
	{
		switch (k)
		{
	...
	TARGET double RELATEDNESS::R_Lynch1999(IND* x, IND* y)
	{
	...
	TARGET double RELATEDNESS::R_Wang2002(IND* x, IND* y)
	{
		if (x->vmax > 2 || x->vmin < 2)
		...
		for (int64 l = 0; l < nloc; ++l)
		{
			...
			if (gx.Nalleles() == 0 || gy.Nalleles() == 0) continue;
			if (cpop->loc_stat1[l].k <= 1) continue;
			LOCSTAT2& stat2 = relatedness_loc_stat2[l];
	...
	TARGET double RELATEDNESS::R_Thomas2010(IND* x, IND* y)
	{
		if (x->vmax > 2 || x->vmin < 2)
	...
	TARGET double RELATEDNESS::R_Li1993(IND* x, IND* y)
	{
		if (x->vmax > 2 || x->vmin < 2)
	...
	TARGET double RELATEDNESS::R_Queller1989(IND* x, IND* y)
	{
		if (x->vmax > 2 || x->vmin < 2)
	...
	TARGET double RELATEDNESS::R_Huang2016A(IND* x, IND* y)
	{
		if (x->vmax > 2 || x->vmin < 2)
	...
	TARGET double RELATEDNESS::R_Huang2016B(IND* x, IND* y)
	{
		if (x->vmax > 2 || x->vmin < 2)
	...
	TARGET void RELATEDNESS::R_AndersonInitialize(IND* x, IND* y)
	{
		if (x->vmax > 2 || x->vmin < 2)
	...
	TARGET double RELATEDNESS::R_Milligan2003(IND* x, IND* y)
	{
		return R_Anderson2007(x, y, false);
	}

	/* Anderson 2007 relatedness estimator */
	TARGET double RELATEDNESS::R_Anderson2007(IND* x, IND* y, bool confine)
	{
	...
	TARGET double RELATEDNESS::L_Anderson(CPOINT& x, void** unusued)
	{
		x.Image2Real();
		double* S = &x.real[0];
	...
	TARGET double RELATEDNESS::R_Ritland1996(IND* x, IND* y, bool ismodified, bool isrelatedness)
	{
		double sr = 0, sw = 0;
	...
	TARGET double RELATEDNESS::R_Loiselle1995(IND* x, IND* y, bool ismodified, bool isrelatedness)
	{
		double sr = 0, sw = 0;
		for (int64 l = 0; l < nloc; ++l)
		{
	...
	TARGET double RELATEDNESS::R_Weir1996(IND* x, IND* y, bool isrelatedness)
	{
		double sr = 0, sw = 0;
		...
		for (int64 l = 0; l < nloc; ++l)
		{
			...
			{
				if (p[i] * stat1.nhaplo <= 1e-5) continue;
				r += (isrelatedness ? minv : 1.0) * (gx.GetFreq(i) * gy.GetFreq(i) - p[i] * p[i]);
				w += p[i] * p[i];
			}
	...
	TARGET double RELATEDNESS::R_Huang2014(IND* x, IND* y)
	{
		double srx = 0, swx = 0, sry = 0, swy = 0;
	...
	TARGET double RELATEDNESS::S_Index(int* c, int* d, int ploidyx, int ploidyy)
	{
		int a[8] = { 0 };
		int b[8] = { 0 };
		SetVal(a, c, ploidyx);
	...
	TARGET int RELATEDNESS::GetRefMode(int* a, int ploidy)
	{
		int score = 0;
	...
	TARGET double RELATEDNESS::HuangMoment(GENOTYPE& gx, GENOTYPE& gy, int64 l, double& weight)
	{
		weight = 0;
		int vx = gx.Ploidy(), vy = gy.Ploidy();
	...
	TARGET void RELATEDNESS::Huang2015_Initialize()
	{
#pragma pack(push, 1)
...
#pragma pack(pop)
	...
	TARGET void RELATEDNESS::Huang2015_Uninitialize()
	{
		delete[] Huang2015_maps;
	}

	/* Huang 2015 relatedness estimator */
	TARGET double RELATEDNESS::R_Huang2015(IND* x, IND* y)
	{
	...
	TARGET double RELATEDNESS::L_Huang2015(CPOINT& xx, void** unusued)
	{
		xx.Image2Real();
	...
	TARGET void RELATEDNESS::Huang2015_MatchAllele(int64 pattern, int* gx, int* gy, int* alleles, int p)
	{
		for (int i = 0; i < 16; ++i)
			alleles[i] = 0;
...
TARGET void CalcRelatedness()
{
	...
	relatedness_loc_stat2 = new LOCSTAT2[nloc];
	...
	for (int k = 1; k <= 3; ++k)
	{
		...
		for (int rl = 0; rl < (k == 2 ? lreg : 1); ++rl)
		{
			...
			for (int i = 0; i < n; ++i)
			{
				...
				cpop->GetLocStat2(relatedness_loc_stat2);
